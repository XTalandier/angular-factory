...
gulp.task('clean', function (cb) {
    return gulp.src([
        ...
        path.join(config.dest, 'apploader'),
        path.join(config.dest, 'js'),
        path.join(config.dest, 'fonts'),
        path.join(config.dest, 'packages')
    ], {read: false})
        .pipe(rimraf());
});
...
gulp.task('fonts', function () {
    return gulp.src(['./src/fonts/*'].concat(config.vendor.fonts))
...
gulp.task('html', function () {
    ...
    gulp.src(['./src/templates/*.html', './src/html/**/*.html', './src/js/**/**/*.html'])
...
/*======================================================================
 =                      Copy AppLoader infos                           =
 ======================================================================*/

gulp.task('apploader', function () {
    streamqueue(
        {objectMode: true},
        gulp.src('./src/apploader/*.js')
    )
        ...
        .pipe(gulp.dest(path.join(config.dest, 'apploader')));

    streamqueue(
        {objectMode: true},
        gulp.src('./src/apploader/*.json')
    )
        .pipe(sourcemaps.init())
        //.pipe(concat('app.js'))
        //.pipe(ngAnnotate())
...
gulp.task('js', function () {
    streamqueue({objectMode: true},
        ...
        gulp.src(['src/templates/**/*.html', './translation/*.js']).pipe(templateCache({module: 'AngularFactory'}))
    )
        .pipe(sourcemaps.init())
        .pipe(concat('app.js'))
        .pipe(ngAnnotate())
...
gulp.task('watch', function () {
    ...
    gulp.watch(['./src/html/**/*', './src/js/**/*.html', './src/templates/*.html'], ['html']);
    gulp.watch(['./src/less/**/*'], ['less']);
    gulp.watch(['./src/js/**/*.js', './src/templates/*.html', config.vendor.js], ['js']);
...
gulp.task('build', function (done) {
    var tasks = ['html', 'fonts', 'images', 'less', 'js', 'apploader'];
